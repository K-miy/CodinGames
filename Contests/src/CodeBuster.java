...
class CodeBuster
{
    ...
    public class AGreedy extends IStrategy
    {
        ...
        {
            Action a = null;
            for (Buster o : CodeBuster.this.teams.get(1 - CodeBuster.this.myTeamId))
            {
                ...
                if (this.b.canStun() && o.isCarrying()) // Je peux stun un carrier adverse
                    if (this.b.lastupd - o.lastupd < 1) // son information est à jour
                        if (o.p.distanceL2(this.b.p) < MAX_STUN && !CodeBuster.this.act.containsValue(toDo))
                        ...
                        {
                            Vector2D oHome = new Vector2D(HOMES[1 - CodeBuster.this.myTeamId]); // other home
                            Vector2D op = new Vector2D(o.p); // old position
                            Vector2D wayBack = oHome.minus(op); // Objective
                            Vector2D ov = wayBack.unitVector().scalarMult(800); // s

                            // estimate position at current turn
                            Vector2D ep = op.plus(ov.scalarMult(this.b.lastupd - o.lastupd));

                            if (new Position(ep).distanceL2(HOMES[1 - CodeBuster.this.myTeamId]) > this.b.p
                                    .distanceL2(HOMES[1 - CodeBuster.this.myTeamId]))// intercept way back
                            {
                                sep(this.b.id + ": INTERCEPTING " + o.id + " !");
                                return new Action(CHIANTS[CodeBuster.this.myTeamId]);
                            }
                        }
            }
        ...
        Action advStunnable(boolean p_occ)
        {
            ...
            // sep(CodeBuster.this.teams.get(1 - CodeBuster.this.myTeamId));
            if (a != null)
                return a;
            ...
            for (Buster o : CodeBuster.this.teams.get(1 - CodeBuster.this.myTeamId))
            {
                ...
                if (!CodeBuster.this.act.containsValue(toDo))
                    if ((this.b.lastupd - o.lastupd) < 1 && o.p.distanceL2(this.b.p) < MAX_STUN && this.b.canStun()
                            && !o.isStun())
                    {
                        if (!p_occ)
                            if ((this.lastA != null && !CHIANTS[CodeBuster.this.myTeamId].equals(this.lastA.moveTo))
                        ...
                        if (this.b.lastupd > 75 && CodeBuster.this.tc.job.get(this.b) != null)
                            sep("##### # #" + this.b.id + " : gid:" + CodeBuster.this.tc.job.get(this.b).id + " sta:"
                                    + CodeBuster.this.tc.job.get(this.b).stamina + "/"
                                    + (CodeBuster.this.tc.job.get(this.b).nbOnIt * 5) + " o : " + o.id + " gid:" + o.ghid
                                    + " t?" + o.isTrapping());

                        if (o.isTrapping() // Les deux catch le meme
                                && CodeBuster.this.tc.job.get(this.b) != null && CodeBuster.this.tc.job.get(this.b).id == o.ghid
                                && CodeBuster.this.tc.job.get(this.b).stamina < (CodeBuster.this.tc.job.get(this.b).nbOnIt * 5))
        ...
        Action backHome()
        {
            ...
            Vector2D hpos = new Vector2D(CodeBuster.this.home);
        ...
        public Action decide()
        {
            ...
            else if (CodeBuster.this.tc.job.get(this.b) != null)
                this.s = AGreedyState.OCCUPE;
            else
                this.s = AGreedyState.LIBRE;

            Ghost g = CodeBuster.this.tc.job.get(this.b);

            // NMumber of my own on g
            int count = 0;
            for (Ghost gg : CodeBuster.this.tc.job.values())
            ...
            switch (this.s)
            {
                case OCCUPE: // Assigné à un fantome non capturé.
                    ...
                    if (d > MAX_BUST || d < MIN_BUST) // Pas à distance
                    {
                        ...
                        if (g.p.equals(this.b.p))
                            gpos = new Vector2D(CodeBuster.this.home);
                    ...
                    {
                        CodeBuster.this.tc.job.remove(this.b); // On se libère de la job
                        CodeBuster.this.ghosts.remove(g.id); // On dit que le ghost n'existe plus jusqu'à qu'on le revoie
                        return this.decide();
                    }
                    ...
                    // plus de 5 tours à 0 ... blocage avec autre team ou recup après stun
                    {
                        ...
                        if (a == null)
                            a = new Action(CHIANTS[CodeBuster.this.myTeamId]);
                    }
                ...
                case CHARGE:
                    ...
                    CodeBuster.this.ghosts.remove(this.b.ghid);
                    ...
                    {
                        a = new Action();
                        CodeBuster.this.tc.job.remove(this.b);
                    }
                ...
                default:// case LIBRE ou STUN: on explore et on stun dès qu'on peut !
                    // Si Wizard Libre (Aucun ghost affecté)
                    a = this.advStunnable(false); // kkun a portée à assomer ?

                    if (a == null)     // sinon on se balade.
                    {
                        this.pToR = CodeBuster.this.ge.nearestNext(this.b.p);
                    ...
                    if (a.moveTo == null && CodeBuster.this.bus.get(a.bustId).isCarrying())
                        CodeBuster.this.tc.job.put(this.b, CodeBuster.this.ghosts.get(CodeBuster.this.bus.get(a.bustId).ghid));
        ...
        boolean noReasonToKeepStun()
        {
            ...
            for (Buster n : CodeBuster.this.teams.get(CodeBuster.this.myTeamId))
                noReason |= (n.p.distanceL2(CHIANTS[CodeBuster.this.myTeamId]) < 50 && n.canStun());
    ...
    public class ATeamCoordinator
    {
        ...
        public void decide(int p_t)
        {
            ...
            for (Buster b : this.team)
                for (Ghost g : CodeBuster.this.ghosts.values())
                {
    ...
    public class Buster extends Entity
    {
        ...
        {
            double h = CodeBuster.this.home.distanceL2(this.p);
         ...
         * Redéfinition.
         *
         * @see CodeBuster.Entity#upd(int, int, int, int, int, int)
         */
        @Override
    ...
    public class GridExploration
    {
        ...
        void upd()
        {
            ...
            for (Position p : this.unexplored)
                for (Buster b : CodeBuster.this.teams.get(CodeBuster.this.myTeamId))
