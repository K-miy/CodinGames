...
class Action implements Comparable<Action>
{
    ...
    public double rollout(State s)
    {
        int i = 150;
        State sn = s;

        ArrayList<Action> poss = new ArrayList<>(s.possibilities());

        double v = 0;

        while (i > 0 && sn != null && !Player5.obj(sn))
        {
            i--;
...
class Player5
{
    ...
    public static boolean insideTerrain(State s)
    {
    ...
    public static boolean obj(State s)
    {
        if (s == null)
            return false;
...
class State
{
    ...
    private static final HashMap<State, HashSet<Action>> MEM = new HashMap<>();
    ...
    public State(Vector2D pp, Vector2D ss, int r, int t, int f)
    {
        this.pos = pp;
        this.sp = ss;
        this.rot = r;
    ...
    public double getReward()
    {
        ...
        Vector2D obj = Player5.flat[0].plus(Player5.flat[1].minus(Player5.flat[0]).scalarMult(0.5));

        boolean hs = this.sp.x < 19 && this.sp.x > -19;
        boolean vs = this.sp.y < 39 && this.sp.y > -39;
        boolean px = this.pos.x > Player5.flat[0].x && this.pos.x < Player5.flat[1].x;
        boolean py = this.pos.y > Player5.flat[0].y && this.pos.y < Player5.flat[1].y + 500;
    ...
    public HashSet<Action> possibilities()
    {
        if (this.pos.x > Player5.flat[0].x && this.pos.x < Player5.flat[1].x && this.sp.x < 19 && this.sp.x > -19)
        {
            int[] actthr = { this.th + 1, this.th, this.th - 1 };
            HashSet<Action> poss = new HashSet<>();
            for (int thr : actthr)
            {

                Action a = new Action(0, thr);
...
class Vector2D extends Point2D.Double
{
    ...
    /** Rounded version of the vector */
    public Vector2D toRound()
    {
