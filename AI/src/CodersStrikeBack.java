...
class CodersStrikeBack
{
    ...
    public boolean continueAllowed()
    {
        this.timer = System.currentTimeMillis() - this.orig;
        if (this.timer > CodersStrikeBack.OOT)
    ...
    void run()
    {
        ...
        while (true)
        {
            ...
            myS = new ScreenNode(this.myGame, this.mySc);
            ...
            // Screen hiS = new Screen(this.hisGame, this.hisSc);
            ...
            // // System.err.println(sb.toString());
            //
            // Screen myS = new Screen(this.myGame, this.mySc);
            //
            // if (this.tour == 1)
    ...
    {
        testing = true;
        CodersStrikeBack p = new CodersStrikeBack();
        ...
        Screen myS = p.new Screen(testG, 0);
    ...
    class Screen implements Comparable<Screen>
    {
        ...
        private Screen(String[] game, int p_sc)
        {
        ...
        public Screen next(Action a)
        {
            ...
            // Réduction de l'état avec les couleurs par groupe de 4 jusqu'à permanence
            while (!nr.equals(n))
            {
                sc.chainCount++;
                n = nr;
        ...
        public Action[] probableActionSet(Pair p, int vMax)
        {
            ...
            int v = 0;
            for (int i = 7; i >= 0 && bL.size() < BRANCHING_FACTOR && v < vMax; i--)
            {
                bL.addAll(m.get(i));
                if (bL.size() > 0)
                    v++;
            ...
            if (bL.size() == 0)
            {
                CodersStrikeBack.this.sep("heurCol.Out:" + v + "/" + bL.size() + "\n" + m);
                CodersStrikeBack.this.sep(this + "" + p);
            }
    ...
    class ScreenNode implements Comparable<ScreenNode>
    {
        ...
        private ScreenNode(Screen p_s, int p_d)
        {
            this.s = p_s;
            this.depth = p_d;
        }

        public ScreenNode(String[] p_myGame, int p_sc)
        {
            this.s = new Screen(p_myGame, p_sc);
        }

        public void aStar(Pair[] p)
        {
            PriorityQueue<ScreenNode> open = new PriorityQueue<>();
            PriorityQueue<ScreenNode> closed = new PriorityQueue<>();
            open.add(this);

            while (CodersStrikeBack.this.continueAllowed() || testing)
            {
                ScreenNode best = open.poll();

                for (Action a : best.s.probableActionSet(p[best.depth], 3))
                {
                    ScreenNode n = new ScreenNode(this.s.nextAndHeuristic(a), this.depth + 1);
        ...
        public Entry<ScreenNode, Action> bestEntry()
        {
            ...
            // CodersStrikeBack.this.sep("bestEntry.In:" + this.next.size());
            ...
            // CodersStrikeBack.this.sep("bestEntry.Out:" + b);
            return b;
        }
        ...
        {
            // CodersStrikeBack.this.sep("devRec.In" + a.size());

            if (p.isEmpty())
                return;

            if (!testing)
                if (!CodersStrikeBack.this.continueAllowed())
                    throw new OutOfTimeException("");

            Pair ip = p.pollFirst();

            for (Action a : this.s.probableActionSet(ip, 1))
            {
                ScreenNode n = new ScreenNode(this.s.nextAndHeuristic(a), this.depth + 1);
                if (!this.next.containsKey(n))
                {
            ...
            // CodersStrikeBack.this.sep("devRec.Mid" + this.next.entrySet().size());
            ...
            // CodersStrikeBack.this.sep("DevRec.Out:" + bA + "/" + max + " -> " + this.s.v);
        }
    ...
    class ZeGeneticAlgo
    {
        ...
        public Individual run(Screen ps, Pair[] pp, Individual seed)
        {
            ...
            while (CodersStrikeBack.this.continueAllowed() || testing)
            {
                ...
                // CodersStrikeBack.this.sep("Generation: " + generationCount + " Fittest: " + myPop.getFittest().getFitness());
                myPop = this.evolvePopulation(myPop);
            }
            CodersStrikeBack.this.sep("Generation: " + generationCount + " Fittest: " + myPop.getFittest().getFitness());
            // CodersStrikeBack.this.sep("Generation: " + generationCount);
            // CodersStrikeBack.this.sep("Genes:" + myPop.getFittest());
        ...
        class Individual implements Comparable<Individual>
        {
            ...
            public Individual(Screen orig, Pair[] pp, boolean random)
            {
                ...
                if (random)
                {
                    ...
                    {
                        Action[] c = ss[i].probableActionSet(this.p[i], 3);
            ...
            public Action[] fromPair()
            {
                ...
                // CodersStrikeBack.this.sep(Arrays.toString(this.genes));
            ...
            private void mutate()
            {
                ...
                for (int i = 0; i < a.length; i++)
                {
                    ...
                    if (fromHere)
                    {
                        Action[] c = ss[i].probableActionSet(this.p[i], 1);
                        a[i] = c[r.nextInt(c.length)];
